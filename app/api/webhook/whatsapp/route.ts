...
function extractBotData(history: { role: string; content: string }[], currentBotData: Record<string, any>): Record<string, any> {
...
function resolveStage(currentStage: string, botMessageCount: number, botData: Record<string, any>): string {
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!updatedBotData.nome) {
      const IGNORAR = /^(olá|ola|oi|bom dia|boa tarde|boa noite|ok|sim|não|nao|tudo|tá|ta|entendi|obrigado|obrigada|claro|bia|buzzi|odontologia|dra|fernanda|quero|gostaria|preciso|tenho|pode|qual|quando|como|onde|agendar|consulta|duvida|ajuda|opa|vao|eai|salve|hey|hello)$/i
      const clientMsgs = historyMessages.filter(m => m.from === 'client').map(m => m.text.trim())
      for (const msg of clientMsgs) {
        const match = msg.match(/(?:meu nome [eé]|me chamo|sou o|sou a|pode me chamar de)\s+([A-Za-zÀ-ú]+(?:\s+[A-Za-zÀ-ú]+)?)\b/i)
    ...
    const confirmedData = replyLower.includes('entrar em contato') || replyLower.includes('nossa equipe') || replyLower.includes('confirmo') || replyLower.includes('anotado') || replyLower.includes('agendaremos')
